...
interface ElectronAPI {
  storage: {
    ...
    deleteCustomAdminUser: () => Promise<{success: boolean; error?: string}>;
...
const isElectronAvailable = () => {
  ...
  console.log('DEBUG: isElectronAvailable:', available);
  console.log('DEBUG: window.electronAPI:', typeof window !== 'undefined' ? window.electronAPI : 'window not available');
  return available;
};
...
export class StorageManager {
  ...
  async initialize(): Promise<void> {
    console.log('DEBUG: StorageManager.initialize() called');
    if (isElectronAvailable()) {
      console.log('DEBUG: Electron API is available, checking portable mode');
      try {
        const result = await window.electronAPI!.storage.isPortable();
        console.log('DEBUG: isPortable result:', result);
        ...
        console.error('DEBUG: Error checking portable mode:', error);
        this.isPortableMode = false;
        console.log('Storage mode: IndexedDB (error fallback)');
      }
    } else {
  ...
  async saveProjects(projects: Project[]): Promise<void> {
    console.log('DEBUG: saveProjects called, isPortableMode:', this.isPortableMode);
    if (this.isPortableMode && isElectronAvailable()) {
      console.log('DEBUG: Using Electron storage for saveProjects');
      const result = await window.electronAPI!.storage.saveProjects(projects);
      console.log('DEBUG: saveProjects result:', result);
      if (!result.success) {
        throw new Error(result.error || 'Failed to save projects');
      }
    } else {
      console.log('DEBUG: Using IndexedDB fallback for saveProjects');
  ...
  async deleteCustomAdminUser(): Promise<void> {
    if (this.isPortableMode && isElectronAvailable()) {
      const result = await window.electronAPI!.storage.deleteCustomAdminUser();
      if (!result.success) {
        throw new Error(result.error || 'Failed to delete custom admin user');
      }
    } else {
      // Fallback to existing IndexedDB implementation
      const { deleteCustomAdminUserFromDB } = await import('./db');
      await deleteCustomAdminUserFromDB();
    }
  }
